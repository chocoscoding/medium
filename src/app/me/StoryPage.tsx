  ...
  return (
    <div className="max-w-[1000px] mx-auto mt-12">
      <div className="flex items-center justify-between">
        ...
        <button onClick={MakeNewStory} className="bg-green-600 hover:bg-green-700 px-4 py-[6px] rounded-full text-white text-sm">
          New Story+
        </button>
      </div>
      <div className="flex items-center space-x-6 border-b-[1px] mt-2">
        <Link
          href="/me/drafts"
          className={`${path === "drafts" ? "border-b-[1px] border-neutral-800 opacity-100" : "opacity-60"} text-sm pb-4`}>
          drafts {TotalDrafts}
        </Link>
        <Link
          href="/me/published"
          className={`${path === "published" ? "border-b-[1px] border-neutral-800 opacity-100" : "opacity-60"} text-sm pb-4`}>
          published {TotalPublished}
        </Link>
        <Link
          href="/me/saved"
          className={`${path === "saved" ? "border-b-[1px] border-neutral-800 opacity-100" : "opacity-60"} text-sm pb-4`}>
          saved{TotalSaved}
        </Link>
      </div>
      {path === "drafts" && (
          ...
          {stories.map((story) => {
            ...
            return (
              <Link key={story.id} href={`/story/${story.id}`}>
                <div className="py-5">
                  <div className="text-xl font-semibold mb-5">{H1Element ?? "no title"}</div>
            ...
            return (
              <Link key={story.id} href={`/published/${story.id}`}>
                <div className="py-5">
                  ...
                  <div className="flex items-center space-x-5">
                    <span className="text-sm opacity-60">{new Date(story.updatedAt).toDateString()}</span>
                    <button onClick={() => EditStory(story.id)} className="px-5 py-1 text-sm rounded-full bg-gray-500 text-white">
                      Edit
                    </button>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-4 gap-10 grid-cols-2">
                  <div className="md:col-span-3">
                    ...
                    <div className="flex items-center justify-between mt-6">
                      {story.topics && <span className="bg-neutral-50 px-2 py-1 rounded-full text-[13px]">{story.topics}</span>}
                    </div>
                  </div>
