  ...
  return (
    <div>
      <div className="flex items-center space-x-6 border-b-[1px] text-sm opacity-60">
        {/* conditionally render the tags based on sign in status */}
        {isSignedIn ? (
          <>
            <span onClick={() => setShowPopUp(!showPopup)} className="pb-3">
              <Plus size={20} />
            </span>
            <Link href="/" className={`pb-3 ${tag === null ? "border-b-[1px] border-neutral-950" : ""}`}>
              For you
            </Link>
          ...
          <p className={`pb-3 border-b-[1px] border-neutral-950`}>Latest</p>
        )}
      </div>
      {filteredStories.map((story) => (
        <StoryItem key={story.id} story={story} />
      ))}
      {showPopup && <AddTagComp allTopics={allTopics} setShowPopUp={setShowPopUp} UserTags={UserTags} />}
    </div>
  );
};
